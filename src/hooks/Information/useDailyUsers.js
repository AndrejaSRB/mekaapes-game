import { useQuery } from "react-query";
import axios from "axios";

const PLAUSIBLE_TOKEN = process.env.REACT_APP_PLAUSIBLE_TOKEN;
const PLAUSIBLE_SITE_ID = process.env.REACT_APP_PLAUSIBLE_SITE_ID;

const getDailyUsers = async () => {
  let response = await axios.get(
    `https://plausible.io/api/v1/stats/aggregate?visit=day&site_id=${PLAUSIBLE_SITE_ID}&period=day&metrics=visitors,pageviews`,
    {
      headers: {
        Authorization: `Bearer ${PLAUSIBLE_TOKEN}`,
      },
    }
  );

  let visitors = 0;
  if (
    response?.data?.results?.pageviews?.value !== undefined &&
    response?.data?.results?.pageviews?.value !== null
  ) {
    visitors = response?.data?.results?.pageviews?.value;
  }
  return visitors;
};

const useDailyUsers = () => {
  return useQuery("daily-users", getDailyUsers);
};

export default useDailyUsers;
